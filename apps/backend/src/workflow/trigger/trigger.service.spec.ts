...
describe('TriggerService', () => {
  ...
  describe('webhook trigger', () => {
    it('returns triggeredBy=webhook and preserves input context', () => {
      const node = makeNode('t1', NodeKind.WEBHOOK);
      const result = service.fire(node, { upstream: 'value' });
  ...
  describe('schedule trigger', () => {
    it('returns triggeredBy=schedule with default cron when cron is not set', () => {
      const node = makeNode('t2', NodeKind.SCHEDULE);
      const result = service.fire(node, {});
      expect(result.triggeredBy).toBe('schedule');
      expect(result.cron).toBe('* * * * *');
      expect(typeof result.firedAt).toBe('string');
    });

    it('uses the cron expression from node.data', () => {
  ...
  describe('unknown kind', () => {
    it('returns the input context unchanged', () => {
      const node = makeNode('t4');
      const input = { foo: 'bar' };
      const result = service.fire(node, input);
